...
df_acc_par_mois["label_mois"] = df_acc_par_mois["mois"].map(
    lambda mois: dico_mois[mois]
)
...
df_acc_par_jour["date"] = (
    "2022-"
    + df_acc_par_jour["mois"].astype(str)
    + "-"
    + df_acc_par_jour["jour"].astype(str)
)
df_acc_par_jour["date"] = pd.to_datetime(df_acc_par_jour["date"])
df_acc_par_jour = (
    df_acc_par_jour[["Accident_Id", "date"]].groupby(by="date").count().reset_index()
)

## Dataframe Répartition par gravité
df_usagers_par_gravite = usagers.copy()
df_usagers_par_gravite = (
    df_usagers_par_gravite.loc[(df_usagers_par_gravite["grav"] != -1), "grav"]
    .value_counts()
    .sort_index()
    .reset_index()
)
df_usagers_par_gravite["grav"] = df_usagers_par_gravite["grav"].map(
    lambda gravite: dico_gravite[gravite]
)

## Dataframe Répartition par type de véhicule
df_acc_par_type_vehicule = vehicules.copy()
df_acc_par_type_vehicule = df_acc_par_type_vehicule[
    (df_acc_par_type_vehicule["catv"] != -1)
]
df_acc_par_type_vehicule = (
    df_acc_par_type_vehicule[["Num_Acc", "catv"]]
    .groupby(by="catv")
    .count()
    .reset_index()
)
df_acc_par_type_vehicule["catv"] = df_acc_par_type_vehicule["catv"].map(
    lambda cat: dico_categorie[cat]
)

## Dataframe Répartition par type de trajet
df_acc_par_type_trajet = usagers.copy()
df_acc_par_type_trajet = (
    df_acc_par_type_trajet[["Num_Acc", "trajet"]]
    .groupby(by="trajet")
    .count()
    .reset_index()
)
df_acc_par_type_trajet["trajet"] = df_acc_par_type_trajet["trajet"].map(
    lambda type: dico_type_trajet[type]
)
...
df_acc_par_meteo = (
    df_acc_par_meteo[["Accident_Id", "atm"]].groupby(by="atm").count().reset_index()
)
df_acc_par_meteo = df_acc_par_meteo.loc[(df_acc_par_meteo["atm"] != -1)]
df_acc_par_meteo["atm"] = df_acc_par_meteo["atm"].map(
    lambda code: dico_conditions_atmospherique[code]
)

## Dataframe Répartition des obstacles mobiles heurtés
df_obstacles_heurtes_par_type = vehicules.copy()
df_obstacles_heurtes_par_type = df_obstacles_heurtes_par_type.loc[
    (df_obstacles_heurtes_par_type["obsm"] != -1), ["Num_Acc", "obsm"]
]
df_obstacles_heurtes_par_type = (
    df_obstacles_heurtes_par_type.groupby(by="obsm").count().reset_index()
)
df_obstacles_heurtes_par_type["obsm"] = df_obstacles_heurtes_par_type["obsm"].map(
    lambda code: dico_obstacle[code]
)

## Rapprochement des lieux sur les caractéristiques
# Dataframe pour les lieux
df_carac_lieux = pd.merge(
    caracteristiques,
    lieux[["Num_Acc", "catr"]],
    how="left",
    left_on=["Accident_Id"],
...
df_carac_vehicules = pd.merge(
    df_carac_lieux, vehicules[["Num_Acc", "id_vehicule"]], how="left"
)

# Dataframe pour les véhicules
df_carac_usagers = pd.merge(
    df_carac_lieux, usagers[["Num_Acc", "id_usager", "grav"]], how="left"
)
